  ...
  <b-card
    ...
    <b-form @submit="onSubmit" @reset="onReset" v-if="show">
      <b-form-group
        ...
        <b-form-input id="input-name" :state="nicknameState" type="text" v-model="form.name"/>
      ...
      <b-form-group label-for="input-avatar" label="New avatar:" label-size="lg">
        <input type="file" class="form-control" accept="image/*" id="input-avatar" @change="form.photo">
      </b-form-group>
  ...
  methods: {
    async onSubmit(event) {
      ...
      if (this.nicknameState) {
        ...
        try {
          const response = await request.post(`/users/me/edit?name=${this.form.name}`, formData);
          ...
          this.error_message = "Something went wrong. Refresh page and try again."
        }
      } else {
        this.error_occured = true;
        this.error_message = "Invalid nickname. No spaces, only letters and digits."
      }
    },
    onReset(event) {
      event.preventDefault()
