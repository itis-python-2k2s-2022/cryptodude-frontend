<template>
  <b-nav
    class="navbar navbar-expand-lg bg-dark navbar-dark text-lg-center font-monospace"
  >
    <div class="container-fluid">
      <router-link class="navbar-brand" :to="{ name: 'index' }"
        >CryptoDude</router-link
      >
      <button
        ...
        >
          <li class="nav-item" v-if="$route.name !== 'hot_offers'">
            <router-link
              ...
              aria-current="page"
              :to="{ name: 'hot_offers' }"
              >Hot offers</router-link
            >
          </li>
          <li class="nav-item" v-if="$route.name !== 'cryptocurrencies'">
            <router-link
              id="nav_cryptocurrencies"
              class="nav-link active"
              aria-current="page"
              :to="{ name: 'cryptocurrencies' }"
          ...
          <li class="nav-item dropdown-menu-end" aria-expanded="false">
            ...
            >
              <span v-if="isAuthorized"
                ><b-avatar :src="usrImgSrc" size="1.5rem"></b-avatar>&nbsp;{{
                  profile.name
                }}</span
              >
              <span v-else>Me</span>
            ...
            >
              ...
              <li v-if="!isAuthorized">
                <router-link :to="{ name: 'auth' }" class="dropdown-item"
                  >Sign in</router-link
                >
              </li>
              <li v-if="isAuthorized">
              ...
              <li v-if="isAuthorized">
                <b-link href="#" class="dropdown-item" @click="onLogOutClick"
                  >Log out</b-link
                >
              </li>
            </ul>
